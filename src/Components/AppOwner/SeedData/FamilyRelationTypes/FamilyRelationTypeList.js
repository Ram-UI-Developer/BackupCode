...
const FamilyRelationTypeList = () => {
    ...
    //api handling for get all  family relations
    const getAllHandler = () => {
        setLoading(true)
        getAllByOrgId({
        ...
        getById({
            ...
            .then(
                (res) => {
                    ...
                    setFamilyData(res.data ? res.data : {})
                    setView(true)
                    setLoading(false)
                },
                (error) => {
    ...
    //api handling for saving family relation data with validation
    const onSaveHandler = () => {
        const obj = {
            id: formData.id,
            organizationId: userDetails.organizationId == null ? 0 : userDetails.organizationId,
    ...
    //api handling for updating family relation data with validation
    const onUpdateHandler = () => {
        ...
        if (!formData.name || formData.name == undefined) {
            setFormErrors(validate(formData))
        } else if (updateValidation(familyData, formData)) {
    ...
    //api handling for delete object
            ...
            <div>
                    ...
                    <div className="container-fluid">
                        <div className="row">
                            <div className="col-md-12">
                                <div className="  ">
                                    <PageHeader pageTitle="Family Relations" />
                                    <div className="">
                                        <Button
                                            className="addButton"
                                            variant="addbtn"
                ...
                <Modal show={view} size="lg" onHide={handleClose}>
                    <Modal.Header closeButton={handleClose}>
                        <Modal.Title>
                            {mode == 'create' ? 'Add Family Relation' : 'Update Family Relation'}
                        </Modal.Title>
                    </Modal.Header>
                    <Modal.Body>
                        {loading ? <DetailLoader /> : ''}
...
export default FamilyRelationTypeList
