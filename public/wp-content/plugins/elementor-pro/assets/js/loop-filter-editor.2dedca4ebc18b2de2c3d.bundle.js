  ...
  \*********************************************************/
            /***/ (__unused_webpack_module, exports, __webpack_require__) => {
                ...
                var _taxonomyFilter = _interopRequireDefault(
                    __webpack_require__(
                        /*! ./taxonomy-filter */ '../modules/loop-filter/assets/js/editor/taxonomy-filter.js'
                    )
                )
                ...
                module.exports = ElementEditorModule.extend({
                    ...
                    onInit() {
                        elementor.channels.editor.on(
                            'editor:widget:taxonomy-filter:section_taxonomy_filter:activated',
                            this.onTaxonomyFilterSectionActive
                        )
                    },
                    onTaxonomyFilterSectionActive() {
                        this.updateSelectedElementOptions()
                        const selectedElementControlView =
                                this.getEditorControlView('selected_element'),
                            selectedElementControlValue =
                                selectedElementControlView.getControlValue()
                    ...
                    updateSelectedElementOptions() {
                        const selectedElementControlView =
                            ...
                            selectedElementControlValue =
                                selectedElementControlView.getControlValue(),
                            selectedElement = !!selectedElementControlValue
                                ? elementor.$previewContents[0].querySelector(
                                      `[data-elementor-id="${elementor.config.document.id}"] .elementor-element-${selectedElementControlValue}`
                                  )
                                : ''
                        if (!selectedElement) {
                            selectedElementControlView.setValue('')
                        }
                        const loopWidgets = elementor.$previewContents[0].querySelectorAll(
                                `[data-elementor-id="${elementor.config.document.id}"] .elementor-widget-loop-grid`
                            ),
                        ...
                        for (const loopWidget of loopWidgets) {
                            selectedElementControlOptions[loopWidget.dataset.id] =
                                `${__('Loop Grid', 'elementor-pro')} ${index++}`
                        }
                        this.updateOptions('selected_element', selectedElementControlOptions)
                    },
                    ...
                    getPostSourceQueryPostType(loopWidgetContainer) {
                        const querySource =
                            loopWidgetContainer.settings.attributes.post_query_post_type
                        let queryPostType = ''
                        switch (querySource) {
                            case 'current_query':
                    ...
                    updateTaxonomyOptions(loopWidgetId) {
                        ...
                        const postTypeTaxonomies = this.getPostTypeTaxonomies(postType)
                            .then((response) => {
                    ...
                    fetchPostTypeTaxonomies(postType) {
                        return fetch(
                            `${elementorCommon.config.urls.rest}elementor-pro/v1/get-post-type-taxonomies`,
                            {
                                method: 'POST',
                                headers: {
                    ...
                    displayErrorDialog() {
                        elementorCommon.dialogsManager
                            .createWidget('alert', {
                                ...
                                message: __(
                                    'We are experiencing technical difficulties on our end. Please try again to reconnect.',
                                    'elementor-pro'
                                ),
                                position: {
                                    my: 'center center',
