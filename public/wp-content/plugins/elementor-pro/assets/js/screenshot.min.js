...
;(() => {
    ...
    class Screenshot extends elementorModules.ViewModule {
        ...
        getDefaultElements() {
            const e = jQuery(ElementorScreenshotConfig.selector),
                t = e.find(
        ...
        captureScreenshot() {
            return (
                this.elements.$elementor.length ||
        ...
        loadExternalCss() {
            ...
            jQuery('link')
                ...
                .each((e, t) => {
                    const s = jQuery(t),
                        n = s.clone()
                    ;(n.attr('href', this.getScreenshotProxyUrl(s.attr('href'))),
                        this.elements.$head.append(n),
                        s.remove())
                })
        }
        loadExternalImages() {
            const e = this.getSettings('external_images_urls')
                .map((e) => `img[src^="${e}"]`)
                .join(', ')
            jQuery(e).each((e, t) => {
                const s = jQuery(t)
                s.attr('src', this.getScreenshotProxyUrl(s.attr('src')))
            })
        }
        handleIFrames() {
            this.elements.$elementor.find('iframe').each((e, t) => {
                const s = jQuery(t),
                    n = jQuery('<div />', {
                        css: { background: 'gray', width: s.width(), height: s.height() }
                    })
                ;(s.before(n), s.remove())
            })
        }
        removeUnnecessaryElements() {
            ...
            ;(this.elements.$sections
                .filter((t, s) => {
                    let n = !1
                    return (
                        e >= this.getSettings('crop.height') && (n = !0),
                        (e += jQuery(s).outerHeight()),
                        n
                    )
                })
                .each((e, t) => {
        ...
        createImage() {
            ...
            return Promise.race([e, t]).then(() => {
                if ((this.log('Start creating screenshot.'), this.getSettings('isDebugSvg')))
                    return (
                        domtoimage
                            .toSvg(document.body, {
                                imagePlaceholder: this.getSettings('image_placeholder')
                            })
                            .then((e) => this.download(e)),
                        Promise.reject('Debug SVG.')
                    )
                return /^((?!chrome|android).)*safari/i.test(window.userAgent)
                    ? (this.log('Creating screenshot with "html2canvas"'),
                      html2canvas(document.body).then((e) => e.toDataURL('image/png')))
        ...
        download(e) {
            const t = jQuery('<a/>', { href: e, download: 'debugSvg.svg', html: 'Download SVG' })
            ;(elementorCommon.elements.$body.append(t), t.trigger('click'))
        }
        ...
        cropCanvas(e) {
            const t = this.getSettings('crop.width'),
                s = this.getSettings('crop.height'),
                n = document.createElement('canvas'),
                i = n.getContext('2d'),
                o = t / e.width
            return (
                (n.width = t),
                (n.height = s > e.height ? e.height : s),
                i.drawImage(e, 0, 0, e.width, e.height, 0, 0, e.width * o, e.height * o),
                Promise.resolve(n)
            )
        }
        save(e) {
            return new Promise((t, s) => {
                elementorCommon.ajax.addRequest('screenshot_save', {
                    data: {
                        ...
                        screenshot: e.toDataURL('image/png')
                    },
                    success: (e) => {
                        ;(this.log(`Screenshot created: ${encodeURI(e)}`), t(e))
                    },
                    error: () => {
                        ;(this.log('Failed to create screenshot.'), s())
                    }
                })
            })
        }
        markAsFailed() {
            return new Promise((e, t) => {
                elementorCommon.ajax.addRequest('screenshot_failed', {
                    ...
                    success: () => {
                        ;(this.log('Marked as failed.'), e())
                    },
                    error: () => {
                        ;(this.log('Failed to mark this screenshot as failed.'), t())
                    }
                })
            })
        }
        getScreenshotProxyUrl(e) {
            return `${this.getSettings('home_url')}?screenshot_proxy&nonce=${this.getSettings('nonce')}&href=${e}`
        }
        screenshotSucceed(e) {
            this.screenshotDone(!0, e)
        }
        screenshotFailed(e) {
            ;(this.log(e, null), this.markAsFailed().then(() => this.screenshotDone(!1)))
        }
        screenshotDone(e, t = null) {
            ;(clearTimeout(this.timeoutTimer),
                ...
                this.log(`Screenshot ${e ? 'Succeed' : 'Failed'}.`, 'timeEnd'))
        }
        log(e, t = 'timeLog') {
            this.getSettings('isDebug') &&
                (console.log('string' == typeof e ? `${this.getSettings('post_id')} - ${e}` : e),
                t && console[t](this.getSettings('timer_label')))
        }
    }
    jQuery(() => {
        new Screenshot()
