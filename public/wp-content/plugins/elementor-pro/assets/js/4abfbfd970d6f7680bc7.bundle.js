...
;(self['webpackChunkelementor_pro'] = self['webpackChunkelementor_pro'] || []).push([
    ['modules_query-control_assets_js_editor_template-query-control_js'],
    {
  ...
  \***************************************************************************/
            /***/ (__unused_webpack_module, exports, __webpack_require__) => {
                ...
                var _queryControl = _interopRequireDefault(
                    __webpack_require__(
                        /*! ./query-control */ '../modules/query-control/assets/js/editor/query-control.js'
                    )
                )
                class TemplateQueryControl extends _queryControl.default {
                    ...
                    createTemplate() {
                        if (!this.confirmSaveBeforeTemplateCreateDialog) {
                            this.confirmSaveBeforeTemplateCreateDialog =
                    ...
                    async createAndSetTemplate() {
                        const controlId = this.model.get('name'),
                            newTemplateType =
                                this.options.container.controls[controlId].actions.new
                    ...
                    getTemplateSourceTypeValue() {
                        if ('repeater' === this.options?.container?.args?.type) {
                            return (
                                this.options.container.renderer.args.settings.attributes._skin ||
                                undefined
                            )
                        }
                        return this.options.container.controls._skin
                            ? this.options.container.panel.getControlView('_skin').getControlValue()
                            : undefined
                    }
                    ...
                    async switchDocument(id) {
                        ...
                        if (
                            document.config.container_attributes &&
                            document.config.container_attributes.class
                        ) {
                    ...
                    async afterAction(context, templateID) {
                        const action =
                            'new' === context
                                ? this.ui.newButton[0].getAttribute('data-after-action')
