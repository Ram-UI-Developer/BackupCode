    ...
    $.extend($.SmartMenus, {
        ...
        prototype: {
            ...
            docClick: function (e) {
                ...
                if (
                    (this.visibleSubMenus.length && !$.contains(this.$root[0], e.target)) ||
            ...
            menuPosition: function ($sub) {
                var $a = $sub.dataSM('parent-a'),
                    ...
                    rightToLeft =
                        ...
                        (!this.opts.rightToLeftSubMenus && $li.is('[data-sm-reverse]')),
                    subOffsetX =
                        level == 2 ? this.opts.mainMenuSubOffsetX : this.opts.subMenusSubOffsetX,
                    subOffsetY =
                        level == 2 ? this.opts.mainMenuSubOffsetY : this.opts.subMenusSubOffsetY,
                    x,
                    y
                if (horizontalParent) {
                ...
                if (this.opts.keepInViewport) {
                    ...
                    if (
                        ...
                        if (this.isFixed()) {
                            ...
                            $sub.css({ 'touch-action': 'none', '-ms-touch-action': 'none' }).on(
                                getEventsNS(events, eNS)
                            )
                        }
                    }
                }
            ...
            menuScroll: function ($sub, once, step) {
                ...
                if (
                    mouse &&
                    ((data.up && data.y > data.downEnd) || (!data.up && data.y < data.upEnd))
                ) {
            ...
            menuScrollRefreshData: function ($sub) {
                var data = $sub.dataSM('scroll'),
                    upEnd =
                        $(window).scrollTop() - $sub.dataSM('parent-a').offset().top - data.itemH
