                ...
                class GridContainer extends elementorModules.frontend.handlers.Base {
                    ...
                    initLayoutOverlay() {
                        ...
                        if (isGridContainer && this.elements?.emptyView) {
                            this.elements.emptyView.style.display = this.shouldRemoveEmptyView()
                                ? 'none'
                                : 'block'
                        }
                    ...
                    getCorrectContainer() {
                        const container = this.elements.container,
                            ...
                            {
                                selectors: { boxedContainer }
                            } = getDefaultSettings
                        this.elements.outlineParentContainer =
                            container.querySelector(boxedContainer) || container
                    }
                    ...
                    getControlValues(control, device, property) {
                        ...
                        if (
                            ('custom' === unit &&
                                'string' === typeof controlValueForCurrentDevice) ||
                            size < computedStyleLength
                        ) {
                            controlData = {
                                value: controlValue
                            }
                    ...
                    onElementChange(propertyName) {
                        ...
                        propsThatTriggerGridLayoutRender = this.getResponsiveControlNames(
                            propsThatTriggerGridLayoutRender
                        )
                    ...
                    isControlThatMayAffectEmptyViewHeight(propertyName) {
                        return (
                            0 === propertyName.indexOf('grid_rows_grid') ||
                            0 === propertyName.indexOf('grid_columns_grid') ||
                            0 === propertyName.indexOf('grid_auto_flow')
                        )
                    }
                    ...
                    updateEmptyViewHeight() {
                        if (this.shouldUpdateEmptyViewHeight()) {
                            const { emptyView } = this.elements,
                                currentDevice =
                                    elementor.channels.deviceMode.request('currentMode'),
                                elementSettings = this.getElementSettings(),
                                gridRows =
                                    'desktop' === currentDevice
                                        ? elementSettings.grid_rows_grid
                                        : elementSettings.grid_rows_grid + '_' + currentDevice
                            emptyView?.style.removeProperty('min-height')
                            if (
                                this.hasCustomUnit(gridRows) &&
                                this.isNotOnlyANumber(gridRows) &&
                                this.sizeNotEmpty(gridRows)
                            ) {
                    ...
                    shouldRemoveEmptyView() {
                        const childrenLength =
                            this.elements.outlineParentContainer.querySelectorAll(
                    ...
                    getMaxOutlineElementsNumber() {
                        const childrenLength =
                            ...
                            { grid_auto_flow: gridAutoFlow } = this.getElementSettings()
                        const flowTypeField = 'row' === gridAutoFlow ? 'columns' : 'rows'
                        const maxElementsByItems =
                            Math.ceil(childrenLength / gridDimensions[flowTypeField].length) *
                            gridDimensions[flowTypeField].length
                        return maxElementsBySettings > maxElementsByItems
                            ? maxElementsBySettings
                            : maxElementsByItems
                    }
                    getMaxElementsNumber() {
                        const elementSettings = this.getElementSettings(),
                            ...
                            gridDimensions = this.getDeviceGridDimensions()
                        if ('row' === gridAutoFlow) {
                            const rows = elementorFrontend.utils.controls.getResponsiveControlValue(
                                elementSettings,
                                'grid_rows_grid',
                                'size',
                                device
                        ...
                        const columns = elementorFrontend.utils.controls.getResponsiveControlValue(
                            elementSettings,
                            'grid_columns_grid',
                            'size',
                            device
            ...
            /***/ (__unused_webpack_module, exports) => {
                ...
                class HandlesPosition extends elementorModules.frontend.handlers.Base {
                    ...
                    isFirstContainer() {
                        return (
                            this.$element[0] ===
                            document.querySelector('.elementor-edit-mode .e-con:first-child')
                        )
                    }
                    isOverflowHidden() {
