...
;(function ($, global) {
    ...
    var DialogsManager = {
        ...
        addWidgetType: function (typeName, properties, Parent) {
            ...
            return (this.widgetsTypes[typeName] = this.createWidgetType(
                typeName,
                properties,
                Parent
            ))
    ...
    DialogsManager.Widget = function (widgetName) {
        ...
        var hideOnOutsideClick = function (event) {
            if (
                isContextMenuClickEvent(event) ||
                $(event.target).closest(elements.widget).length ||
                isIgnoredTarget(event)
            ) {
                return
            }
        ...
        var initElements = function () {
            ...
            if (settings.closeButton) {
                ...
                const $button = $('<a>', settings.closeButtonOptions.attributes),
                    $buttonIcon = $(settings.closeButtonOptions.iconElement).addClass(
                        settings.closeButtonOptions.iconClass
                    )
            ...
            elements.widget.addClass(classes.join(' ')).attr({
                'aria-modal': true,
                role: 'document',
                tabindex: 0
            })
    ...
    DialogsManager.addWidgetType('buttons', {
        ...
        addButton: function (options) {
            ...
            var $button = self.addElement(
                options.name,
                $('<' + buttonSettings.tag + '>').html(options.text),
                classes
            )
    ...
    DialogsManager.addWidgetType(
        'lightbox',
        DialogsManager.getWidgetType('buttons').extend('lightbox', {
            getDefaultSettings: function () {
                var settings = DialogsManager.getWidgetType(
                    'buttons'
    ...
    DialogsManager.addWidgetType(
        'confirm',
        DialogsManager.getWidgetType('lightbox').extend('confirm', {
            onReady: function () {
