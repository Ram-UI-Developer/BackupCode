...
;(function (root, doc) {
    ...
    function initToolbar(ctx) {
        ...
        if (!ctx._toolbar) {
            ...
            if (
                toolListValues.indexOf('createlink') >= 0 ||
                toolListValues.indexOf('insertimage') >= 0
            )
                icons += inputStr
    ...
    InlineEditor = function (config) {
        ...
        if (defaults.placeholder)
            editor.setAttribute(this.config.placeholderAttr, defaults.placeholder)
    ...
    InlineEditor.prototype.execCommand = function (name, value) {
        ...
        if (commandsReg.block.test(name)) {
            ...
            utils.log(
                'can not find command function for name: ' +
                    name +
                    (value ? ', value: ' + value : ''),
                true
            )
    ...
    InlineEditor.prototype.destroy = function () {
        ...
        config.editor.classList.remove.apply(
            config.editor.classList,
            config.class.split(' ').concat(config.placeholderClass)
        )
    ...
    InlineEditor.prototype.toMd = function () {
        var html = this.getContent()
            .replace(/\n+/g, '') // remove line break
            .replace(/<([uo])l\b[^>]*>(.*?)<\/\1l>/gi, '$2') // remove ul/ol
